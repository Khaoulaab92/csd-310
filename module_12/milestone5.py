...
#Tanner Agnew
#07/20/2023
#Module 12.2 Assignment: Milestone 5
...
cursor = db.cursor()
cursor.execute("SELECT equipment_name, equipment_price, COUNT(*) AS 'Equipment Sales Count' FROM sale JOIN equipment ON sale.equipment_id = equipment.equipment_id WHERE sale_type = 'purchased' GROUP BY equipment_name, equipment_price")
...
# Display Booking Trend Report details
cursor = db.cursor()
cursor.execute("SELECT trip_destination, start_date, COUNT(*) AS 'Booking Count' FROM trip JOIN booking ON trip.trip_id = booking.trip_id GROUP BY trip_destination, start_date")
bookings = cursor.fetchall()
print("-- DISPLAYING Booking Trend Report Details --\n")
for booking in bookings:
